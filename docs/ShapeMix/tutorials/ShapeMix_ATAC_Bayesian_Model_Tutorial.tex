...
\vfill
{\large Progressive rewrite, version 2.2\par}
...
\begin{watchout}
\textbf{Notation update in version 2.2.} The fixed reference peak rate is now
written \(R\), because \textbf{R}eference and \textbf{R}ate both start with R;
versions up to 2.1 called it \(A\). To free the letter, the raw
training-reference cut-site counts, previously \(R_{i,p,b}\), are now
\(x_{i,p,b}\). The specification and the software still use the older letters
(\texttt{A} for the rate, \texttt{R} for the raw reference counts).
\end{watchout}
...
\begin{checkpoint}
If \(R=5\) cut sites per unit and \(N\) is near 20, what effective abundance is
...
\textbf{Answer:} \(R_T=R_B\), so moving one unit from T to B leaves the predicted
total unchanged.
\end{checkpoint}
...
\(R_T=R_B\), the total cannot distinguish T from B, but the short/mono/long split
can.
\end{bigidea}
...
Adding \(p\) turns \(R_T\) into \(R_{T,p}\) and \(\mu\) into \(\mu_p\):
\[
\mu_p=z_TR_{T,p}+z_BR_{B,p}.
\]

\begin{center}
\begin{tabular}{C{0.14\linewidth}C{0.20\linewidth}C{0.20\linewidth}L{0.30\linewidth}}
\toprule
Peak & \(R_{T,p}\) & \(R_{B,p}\) & Teaching purpose \\
\midrule
...
\midrule
reference peak rates \(R\) & observed counts \(N,Y\) \\
...
\begin{itemize}[leftmargin=2em]
  ...
  \item \(R_{T,12}\): fixed T-reference total rate at peak 12.
...
\(R\) & \(C\times P\) & fixed type-by-peak rates \\
...
v_{s,p,b}
=
\sum_cz_{s,c}R_{c,p}\omega_{c,p,b}.
}
\]
...
\(R,\omega,\phi_{\mathrm{ref}}\) are shared by all spots and are left implicit in
...
\begin{tikzpicture}[>=Latex,every node/.style={align=center,font=\small}]
  \node[draw,rounded corners,fill=fixedpale,minimum width=3.3cm,minimum height=1.35cm] (ref) at (0,1.0)
    {\textbf{fixed rates + shapes}\\\(R,\omega\)};
...
\begin{enumerate}[leftmargin=2em]
  ...
  \item Combine \(z\) with the fixed reference rates and shapes \(R,\omega\) to
...
\(R,\omega\) & fixed reference information that turns a mixture into count and shape predictions \\
...
\toprule
Peak & T rate \(R_{T,p}\) & B rate \(R_{B,p}\) & observed \(N_p\) \\
\midrule
1 & 12 & 12 & 24 \\
2 & 8 & 16 & 20 \\
\bottomrule
...
estimation formulas. Its purpose is to show why \(R\), \(\omega\), and
...
\begin{newvar}{\(\obsq{x_{i,p,b}}\) and \(\obsq{T_{i,p}}\)}
\(x_{i,p,b}\) is the training-reference cut-site count for cell \(i\), peak \(p\),
and bin \(b\).\\
\(T_{i,p}=\sum_bx_{i,p,b}\) is that reference cell's total at the peak.\\
\textbf{Letter:} \(x\) is statistics' usual letter for raw data values.
\end{newvar}

If reference cell 17 is labeled T and \(x_{17,42,\mathrm{short}}=3\), that cell
...
\midrule
\endhead
\(R\) & reference peak rate & fixed reference input \\
...
\(x_{i,p,b}\) & training-reference bin count & observed reference data \\
\(T_{i,p}\) & training-reference peak total & observed, calculated from \(x\) \\
\bottomrule
\end{longtable}
...
\[
\begin{aligned}
v_{s,p,b}&=\sum_cz_{s,c}R_{c,p}\omega_{c,p,b}, &
\mu_{s,p}&=\sum_bv_{s,p,b}, &
\rho_{s,p,b}&=\frac{v_{s,p,b}}{\mu_{s,p}}.
\end{aligned}
\]
...
R_{c,p}
=
\frac{\sum_{i\in\mathcal I_c}T_{i,p}+\alpha_Rg_p}
{\sum_{i\in\mathcal I_c}h_i+\alpha_R}.
\]
...
u_{p,b}
=
\frac{\sum_ix_{i,p,b}+\alpha_\omega u_b^{\mathrm{global}}}
{\sum_{i,b'}x_{i,p,b'}+\alpha_\omega},
\]
then smooth each cell-type/peak fingerprint:
\[
\omega_{c,p,b}
=
\frac{\sum_{i\in\mathcal I_c}x_{i,p,b}
+\alpha_\omega u_{p,b}}
...
=
\log p(N\mid z,R,\phi_{\mathrm{ref}})
+\log p(z),
\]
\[
...
\begin{enumerate}[leftmargin=2em]
  ...
  \item Fixed reference rates \(R\) predict how many cut sites each type contributes.
