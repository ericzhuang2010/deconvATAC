...
- Parameterize `z = exp(raw_z) + epsilon` so Adam updates abundance on a
  scale-invariant log coordinate.
...
The implementation freezes the remaining optimizer details before primary runs. Collapsed-count NNLS supplies the starting abundance with each value clipped to `0.05`; an all-zero or invalid NNLS result uses a uniform positive fallback. Restart zero is the exact NNLS abundance in log coordinates. Each later restart `r` receives the multiplicative perturbation `exp(N(0, 0.20))` from `PCG64(SeedSequence([20260822, outer_split_seed, inner_mixture_seed, 0, r]))`. Convergence is reached when the complete objective fails to improve its anchor by `tolerance * max(1, abs(anchor))` for `patience` consecutive steps, or when the full-gradient L2 norm is at most `tolerance`. Reaching `max_steps` alone is not convergence. Each restart retains its best finite state, and the fit selects the largest complete objective only among finite, converged restarts.

Optimizer-coordinate amendment, 2026-08-25: the first GSE129785 execution
completed all 16 NNLS jobs, but all 32 ShapeMix jobs reached `max_steps`
without convergence and produced no ShapeMix proportions. Initialization and
trajectory diagnostics showed effective NNLS abundances as large as 4,566.9;
the former softplus coordinate made the frozen Adam step size unable to move
that scale adequately. The statistical likelihood, priors, signatures,
dispersion, seeds, stopping rule, and compute budget remain unchanged. Only
the optimizer coordinate changed to `z = exp(raw_z) + epsilon`, restart zero
became the exact NNLS point, and failed fits gained a hashed
`failure_diagnostics.json`. A one-descriptor development check subsequently
converged in both arms (selected steps 1,230 shape-aware and 432 count-only).
It was inspected only for convergence and finite normalized output, not for
endpoint selection or parameter tuning. Evidence is retained under
`results/development/shapemix_gse129785_convergence_v1/`; the failed external
attempt remains under its original run group. Production external results
must use a fresh run group, and the changed optimizer must pass a fresh CUDA
qualification before any CUDA-routed production campaign.
...
**Status 2026-08-25:** the original v1 CPU batch finalized 16 NNLS jobs but all
32 ShapeMix jobs failed convergence and produced no ShapeMix predictions. Keep
that batch immutable. The replacement configuration
`configs/experiments/shapemix_gse129785_external_v2.yaml` passes the layout
gate and writes to the fresh
`results/external_validation/shapemix_gse129785_v2/` scope. Launch it only
after the focused failure-diagnostics test and post-amendment verification
remain clean.
...
results/development/shapemix_gpu_qualification_v2/
...
Qualification amendment, made on 2026-08-25 before any external prediction:
the original strict thresholds and observed failures remain visible in the
qualification table. The 32 by 200 smoke input remains on CPU because its
CPU/CUDA maximum was `0.00347122` and CUDA speedup was only `1.025x`. The
full-size platform bound is amended to `2e-4`: only 3/16,384 proportion
differences exceeded `1e-4`, the maximum was `0.000157089`, the metric maximum
was `3.18e-8`, repeated and cached/streamed CUDA results were identical, and
cached CUDA speedup was `4.474x`. All GSE129785 inputs use paired CPU configs;
represented full-size workloads use paired CUDA configs.
