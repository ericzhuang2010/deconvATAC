...
6. [Additional dataset plan](datasets/additional_datasets.md) — external source selection, storage layout, preprocessing requirements, and acquisition gates.
7. [Dataset descriptions](datasets/dataset_descriptions.md) — biological systems, assay roles, expected comparisons, and preprocessing needs.
8. [GSE129785 preprocessing record](datasets/gse129785_preprocessing.md) — frozen immune scope, commands, cell calls, feature selection, outputs, and validation gates.
9. [GSE205055/GSE263333 preprocessing record](datasets/gse205055_gse263333_preprocessing.md) — complete spatial source scope, hashes, preprocessing outputs, barcode alignment, and reference gates.
10. [GSE194122 preprocessing record](datasets/gse194122_preprocessing.md) — multi-donor BMMC source identities, fragments, barcode bridge, labels/features, and leave-one-donor-out indices.
11. [Full-evaluation report](full_evaluation_report.md) — detailed end-to-end record of datasets, preprocessing, compute, experiments, results, validation, artifacts, and interpretation.
12. [Results summary](results_summary.md) — concise primary outcome, validation, controls, and interpretation limit.
13. [Step 6 execution report](step6_results.md) — detailed non-normative result, diagnostics, gates, provenance, and limitations.
14. [Research proposal](../research_class/ShapeMix_ATAC_proposal_draft.md) — current motivation, research question, and broader study design.
...
Steps 0 through 6 of the implementation plan were completed in this workspace on 2026-08-23. The subsequent full evaluation across the PBMC benchmark, GSE129785, GSE194122, GSE205055, GSE263333, and PBMC stress-v2 campaign was completed and audited on 2026-09-09. In total, 372/372 model runs completed, every run manifest verified, and the final synthesis contains seven campaign entries, 82 effect rows, and 352 resource rows. See the [full-evaluation report](full_evaluation_report.md) for the complete data, execution, numerical-result, validation, and provenance record.
...
Development degeneracy controls passed their exact algebraic checks, and the deterministic permutation diagnostic was frozen. Because the primary direction was not positive, the protocol did not trigger the expensive primary negative-control campaign. NNLS completed the configured primary and external same-data baselines. PBMC stress v2 was completed; Cell2location, RCTD, SpatialDWLS, and any additional unregistered sensitivity campaigns remain explicit future gates, not missing result rows.

One pre-launch shape-aware fit on `pbmc_granulocyte_sorted_10k_shapemix_equal_celltype_split_1103_mix_101` was used as a blinded resource pilot: only dimensions, resource demand, convergence, and output shape were inspected. Its retained artifact records 1,268.645 seconds of fit time but not the thread environment. It remained under `/private/tmp`, outside `results/primary` and every strict summary. The PBMC primary benchmark still contains one donor only; its five outer splits quantify conditional resampling variability, while the completed external campaigns add cross-donor, nominal physical-mixture, stress, and real-spatial evidence. The final repository suite passes with 454 tests and one expected skip.
