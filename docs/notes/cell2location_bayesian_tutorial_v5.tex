...
\hypersetup{
  ...
  urlcolor=deepblue,
  citecolor=deepblue,
  pdftitle={A High-School Friendly Tutorial on the Cell2location Bayesian Model v5},
  pdfauthor={OpenAI},
...
\large
This tutorial explains the page-12 Bayesian model step by step: what is observed, what is unknown, what is being modeled, what the symbols mean, why priors are used, which prior distributions are used for the main parameters, and why the user-adjusted hyperparameters matter.
\end{tcolorbox}
\vfill
{\large Prepared as a learning guide for a high school student\\Version 5}\par
\vspace{0.3in}
{\large \today}\par
\end{titlepage}
...
\section{Which prior distribution does each parameter have?}

You asked an important question: ``For the parameters, should they follow some distribution controlled by hyperparameters?''

The Bayesian answer is: usually, yes. In a Bayesian model, an unknown parameter is usually given a \term{prior distribution}. That prior distribution is often controlled by one or more \term{hyperparameters}. However, in a large model like cell2location, not every symbol in the equation is the same kind of object.

\begin{bigidea}
A simple Bayesian model might look like
\[
\text{parameter}\sim \text{prior controlled by hyperparameters},
\]
then
\[
\text{data}\sim \text{likelihood controlled by the parameter}.
\]
Cell2location follows this idea, but with several layers of parameters. Some parameters have priors directly; others are built from hidden intermediate variables that also have priors.
\end{bigidea}

For this section, I will use the \term{Gamma(shape, rate)} convention:
\[
X\sim \mathrm{Gamma}(a,b)
\quad\Longrightarrow\quad
\E[X]=\frac{a}{b}.
\]
The first number is the \term{shape}; the second number is the \term{rate}. This matters because some books use ``scale'' instead of ``rate.''

\begin{warningbox}
The paper's page-12 Methods page gives the main count model and names the two user-adjusted hyperparameters, $\hat N$ and $\alpha^y$. The full implementation has more internal prior constants than the paper highlights. So, in this section, think of the detailed formulas as a beginner-friendly map of the model's prior structure, not as the only way one could write a Bayesian spatial model.
\end{warningbox}

\subsection{A first map: data, parameters, inputs, and hyperparameters}

The page-12 model is
\[
...
\begin{center}
\begin{tabularx}{0.96\textwidth}{>{\bfseries}p{0.18\textwidth}p{0.22\textwidth}X}
\toprule
Symbol & Role & Plain-English meaning \\
\midrule
$d_{s,g}$ & observed data & the count already measured for gene $g$ in spot $s$ \\
$w_{s,f}$ & parameter & the cell-type abundance we mainly want to estimate \\
$m_g$ & parameter & gene-specific technology correction \\
$y_s$ & parameter & spot-specific RNA detection sensitivity \\
$s_{e,g}$ & parameter & background / contaminating RNA term \\
$\alpha_{e,g}$ & parameter & negative-binomial overdispersion, or extra count noise \\
$g_{f,g}$ & input in this spatial step & reference expression of gene $g$ in cell type $f$ \\
$\hat N$, $\alpha^y$ & hyperparameters & user-adjusted settings that control priors \\
\bottomrule
\end{tabularx}
\end{center}

So your intuition was partly right: if we write a generic Bayesian formula such as
\[
p(\theta\mid D),
\]
then $w_{s,f}$ is the most important part of $\theta$. But in the full model,
\[
\theta=\{w_{s,f},m_g,y_s,s_{e,g},\alpha_{e,g},\text{extra hidden prior variables}\}.
\]

\subsection{Visual ladder: hyperparameter to prior to parameter to data}

\begin{figure}[H]
\centering
\begin{tikzpicture}[node distance=0.9cm, every node/.style={font=\small,align=center}, >=Latex]
  \node[draw, rounded corners, fill=softgreen, text width=3.0cm] (h) {Hyperparameters\\settings such as\\$\hat N$, $\alpha^y$};
  \node[draw, rounded corners, fill=softblue, right=1.1cm of h, text width=3.0cm] (p) {Prior distributions\\such as Gamma\\or Exponential};
  \node[draw, rounded corners, fill=softorange, right=1.1cm of p, text width=3.0cm] (theta) {Unknown parameters\\$w_{s,f}$, $m_g$,\\$y_s$, $s_{e,g}$};
  \node[draw, rounded corners, fill=lightyellow, right=1.1cm of theta, text width=2.8cm] (like) {Likelihood\\$d_{s,g}\sim\NB(\mu,\alpha)$};
  \draw[->, thick] (h) -- (p);
  \draw[->, thick] (p) -- (theta);
  \draw[->, thick] (theta) -- (like);
  \node[below=0.45cm of h, text width=3.0cm] {chosen before fitting};
  \node[below=0.45cm of theta, text width=3.0cm] {estimated from data};
  \node[below=0.45cm of like, text width=2.8cm] {connects model to observed counts};
\end{tikzpicture}
\caption{The role of hyperparameters, priors, parameters, and data in a Bayesian model.}
\end{figure}

\subsection{Observed data: $d_{s,g}$}

This is \term{not} a parameter. This is the measured data.

\[
d_{s,g}=\text{observed count of gene }g\text{ at spatial location }s.
\]
...
\begin{quote}
Once the model chooses possible values for all unknown parameters, it predicts an average count $\mu_{s,g}$. The actual observed count $d_{s,g}$ is then allowed to wiggle around that average using a negative binomial distribution.
\end{quote}

This distribution is called the \term{likelihood}. It is the probability model for the data after the unknown parameters have been chosen.

\subsection{Cell abundance: $w_{s,f}$}

This is the most important parameter for biological interpretation.

\[
w_{s,f}=\text{abundance of cell type }f\text{ at location }s.
\]

This is the closest thing to the ``answer'' that cell2location wants. It asks: how much of each cell type is present at each spot?

But $w_{s,f}$ does not get only a one-line prior such as
\[
w_{s,f}\sim \mathrm{Gamma}(\cdots).
\]
Instead, it receives a \term{hierarchical factorization prior}. That means the prior is built in several steps.

A simplified version is:
\[
n_s\sim \mathrm{Gamma}(\hat N\rho_N,\rho_N),
\]
where $n_s$ represents the total expected number of cells at location $s$ and $\rho_N$ is a prior-strength constant. Because
\[
\E[n_s]=\frac{\hat N\rho_N}{\rho_N}=\hat N,
\]
the hyperparameter $\hat N$ says:

\begin{quote}
Before seeing the RNA data, we expect about $\hat N$ cells in a typical spot.
\end{quote}

Then the model creates hidden tissue-zone variables. A beginner-friendly way to write the idea is:
\[
\bar w_{s,f}=\sum_r z_{s,r}x_{r,f}.
\]
Here:
\begin{itemize}
  \item $z_{s,r}$ says how much tissue zone or compartment $r$ is present at spot $s$.
  \item $x_{r,f}$ says how strongly cell type $f$ belongs to tissue zone $r$.
  \item $\bar w_{s,f}$ is the prior mean for cell type $f$ at spot $s$.
\end{itemize}

Finally, the abundance is modeled as something like
\[
w_{s,f}\sim \mathrm{Gamma}\left(\rho_w\bar w_{s,f},\rho_w\right),
\]
so that
\[
\E[w_{s,f}]=\bar w_{s,f}.
...
\begin{bigidea}
The prior for $w_{s,f}$ is hierarchical because the model first imagines shared tissue zones, then uses those zones to guide cell-type abundance estimates. This lets similar spots borrow information from each other.
\end{bigidea}

So, for $w_{s,f}$:
\[
w_{s,f}\text{ is a parameter,}
\]
while
\[
\hat N, R, \rho_N, \rho_w\text{ and related prior constants are hyperparameters.}
\]

The main user-facing hyperparameter here is $\hat N$.

\subsection{Are $n_s$, $\rho_N$, $z_{s,r}$, and $x_{r,f}$ part of $\theta$?}

This is a very good question because the cell-abundance prior suddenly introduces new symbols. The short answer is:

\[
\boxed{\text{Some of the new symbols are part of the full }\theta,\text{ but not all of them.}}
\]

The clean rule is:
\[
\theta=\text{unknown things the model estimates from the data,}
\]
and
\[
H=\text{fixed settings or hyperparameters chosen before fitting the model.}
\]

So in the cell-abundance hierarchy,
\[
n_s\in \theta_{\mathrm{full}},
\]
but
\[
\rho_N\in H.
...
Here $\rho_N$ is not a cell abundance. It is a prior-strength setting. It helps control how tightly $n_s$ should stay near the prior expectation $\hat N$.

\begin{warningbox}
The symbol $\theta$ can be used in two different ways. In a simple explanation, people may use $\theta$ to mean only the main parameter of interest, $W=\{w_{s,f}\}$. In the full mathematical model, $\theta$ means all unknown quantities, including hidden helper variables such as $n_s$, $z_{s,r}$, and $x_{r,f}$.
\end{warningbox}

A useful full Bayes-rule version is:
\[
p(\theta_{\mathrm{full}}\mid D,H)
=
\frac{p(D\mid \theta_{\mathrm{full}},H)\,p(\theta_{\mathrm{full}}\mid H)}{p(D\mid H)}.
\]
Here
\[
D=\{d_{s,g}\}
\]
is the observed spatial gene-count data, and $H$ contains fixed hyperparameters and model settings.

For cell2location, a more complete version of $\theta$ is something like
\[
\theta_{\mathrm{full}}
=
\{w_{s,f}, n_s, b_s, z_{s,r}, x_{r,f}, k_r,
 m_g, y_s, y_e, s_{e,g}, \alpha_{e,g},\ldots\}.
\]
The dots mean that a real implementation may include additional internal variables. The main thing we care about biologically is still
\[
w_{s,f},
\]
but the model introduces extra hidden variables to estimate $w_{s,f}$ more stably.

\begin{bigidea}
A hierarchical Bayesian model often estimates more unknowns than the final answer you want. Some unknowns are \emph{helper parameters}. They help organize the prior, share information across spots, and avoid overfitting.
\end{bigidea}

\subsubsection*{The classroom analogy}

Suppose you want to estimate how many students from each grade are in each classroom.

\begin{itemize}
  \item $w_{s,f}$: number of students of grade $f$ in classroom $s$.
  \item $n_s$: total number of students in classroom $s$.
  \item $\hat N$: your prior guess that a classroom has about, say, 30 students.
  \item $\rho_N$: how strongly you believe classroom sizes should be close to 30.
\end{itemize}

The model estimates $w_{s,f}$ and $n_s$. But you choose $\hat N$ and $\rho_N$ before fitting the model. That is why $w_{s,f}$ and $n_s$ are parameters, while $\hat N$ and $\rho_N$ are hyperparameters.

\subsubsection*{Why $n_s$ is part of $\theta$, but $\rho_N$ is not}

Recall the simplified prior:
\[
n_s\sim \mathrm{Gamma}(\hat N\rho_N,\rho_N).
\]
...
\E[n_s]=\frac{\hat N\rho_N}{\rho_N}=\hat N.
\]
So $n_s$ is the model's unknown estimate of the total number of cells at spot $s$. Because it is unknown and inferred, it belongs in the full parameter set $\theta_{\mathrm{full}}$.

By contrast, $\hat N$ and $\rho_N$ are fixed knobs controlling the prior:
\[
\hat N=\text{prior expected number of cells per spot,}
\]
\[
\rho_N=\text{strength or tightness of that prior expectation.}
\]
Therefore,
\[
\hat N,\rho_N\in H,
\]
not usually in $\theta_{\mathrm{full}}$.

In the software implementation, $\hat N$ is often called \texttt{N\_cells\_per\_location}, and the quantity playing the role of $\rho_N$ is often called \texttt{N\_cells\_mean\_var\_ratio}. The idea is:
\[
n_s\sim
\mathrm{Gamma}
\left(
\texttt{N\_cells\_per\_location}\times
\texttt{N\_cells\_mean\_var\_ratio},
\texttt{N\_cells\_mean\_var\_ratio}
\right).
\]
The first fixed number sets the approximate expected number of cells per spot; the second fixed number controls how concentrated the prior is around that expectation.

\subsubsection*{The helper variables in the cell-abundance hierarchy}

The reason this is confusing is that $w_{s,f}$ is not given a completely independent prior at every spot. Instead, the model builds $w_{s,f}$ through hidden helper variables. A simplified hierarchy is:

\[
n_s \sim \mathrm{Gamma}(\hat N\rho_N,\rho_N),
\]
\[
b_s \sim \mathrm{Gamma}(B,1),
\]
\[
z_{s,r}\sim
\mathrm{Gamma}
...
\mathrm{Gamma}
\left(
\rho_w\sum_r z_{s,r}x_{r,f},
\rho_w
\right).
\]

Do not worry if this looks like a lot. The structure is more important than the exact algebra:

\[
\text{fixed hyperparameters}
\rightarrow
\text{hidden helper parameters}
\rightarrow
\text{cell abundance }w_{s,f}
\rightarrow
\text{observed gene counts }d_{s,g}.
\]

\begin{figure}[H]
\centering
\begin{tikzpicture}[node distance=0.95cm, every node/.style={font=\small,align=center}, >=Latex]
  \node[draw, rounded corners, fill=softgreen, text width=3.0cm] (H) {Fixed settings\\$H$\\$\hat N,\rho_N,R,A,B,\rho_w$};
  \node[draw, rounded corners, fill=softblue, right=1.0cm of H, text width=3.0cm] (helpers) {Hidden helper parameters\\$n_s,b_s,z_{s,r},x_{r,f},k_r$};
  \node[draw, rounded corners, fill=softorange, right=1.0cm of helpers, text width=2.8cm] (W) {Main abundance\\parameter\\$w_{s,f}$};
  \node[draw, rounded corners, fill=lightyellow, right=1.0cm of W, text width=2.9cm] (D) {Observed data\\$d_{s,g}$};
  \draw[->, thick] (H) -- (helpers);
  \draw[->, thick] (helpers) -- (W);
  \draw[->, thick] (W) -- (D);
  \node[below=0.40cm of H, text width=3.0cm] {chosen before fitting};
  \node[below=0.40cm of helpers, text width=3.0cm] {estimated from data};
  \node[below=0.40cm of W, text width=2.8cm] {main biological answer};
  \node[below=0.40cm of D, text width=2.9cm] {already measured};
\end{tikzpicture}
\caption{In the full model, $\theta$ includes both the main abundance parameter $w_{s,f}$ and hidden helper parameters. Hyperparameters belong to $H$, not $\theta$.}
\end{figure}

\subsubsection*{Which of these symbols are part of $\theta$?}

\begin{center}
\begin{tabularx}{0.97\textwidth}{>{\bfseries}p{0.16\textwidth}p{0.20\textwidth}X}
\toprule
Symbol & Part of full $\theta$? & What it means \\
\midrule
$w_{s,f}$ & Yes & Main cell-type abundance parameter: how much cell type $f$ is in spot $s$. \\
$n_s$ & Yes & Hidden total-cell-abundance variable for spot $s$. \\
$b_s$ & Yes & Hidden variable helping control how abundance is spread across tissue groups at spot $s$. \\
$z_{s,r}$ & Yes & Hidden amount of tissue group $r$ at spot $s$. \\
$x_{r,f}$ & Yes & Hidden loading: how much cell type $f$ belongs to tissue group $r$. \\
$k_r$ & Yes & Hidden variable controlling how spread out the loadings $x_{r,f}$ are. \\
$\hat N$ & No & Hyperparameter: expected cells per spot. \\
$\rho_N$ & No & Hyperparameter: strength/tightness of the prior around $\hat N$. \\
$R$ & No & Fixed number of tissue groups. \\
$A,B,\rho_w$ & No & Fixed prior constants controlling the hierarchy. \\
\bottomrule
\end{tabularx}
\end{center}
...
\[
\boxed{\text{$w_{s,f}$ is the main parameter of interest.}}
\]
\[
\boxed{\text{$n_s,z_{s,r},x_{r,f},\ldots$ are also latent parameters in the full model.}}
\]
\[
\boxed{\text{$\hat N,\rho_N,\alpha^y,R,\ldots$ are hyperparameters or fixed model settings.}}
\]

\subsubsection*{A narrow $\theta$ versus a full $\theta$}

Sometimes authors write a narrow parameter of interest:
\[
\theta=W=\{w_{s,f}\}.
\]
This is okay when $W$ is the only thing we want to discuss. But mathematically, the model still contains many other unknowns. Then it is clearer to write
\[
\theta_{\mathrm{full}}=(W,\eta),
\]
where
\[
\eta=\{n_s,b_s,z_{s,r},x_{r,f},k_r,m_g,y_s,y_e,s_{e,g},\alpha_{e,g},\ldots\}.
\]
Here $\eta$ means ``all the other unknown parameters.''

If we only want $W$, Bayesian inference averages over the nuisance variables:
\[
p(W\mid D,H)=\int p(W,\eta\mid D,H)\,\dd\eta.
\]
That integral is just a compact mathematical way to say:

\begin{quote}
Consider all reasonable values of the helper parameters, weight them by how believable they are after seeing the data, and summarize what they imply about $W$.
\end{quote}

\begin{checkpoint}
For the cell-abundance hierarchy, remember this: $n_s$ is estimated, so it is part of the full parameter set. $\rho_N$ is chosen to control a prior, so it is a hyperparameter.
\end{checkpoint}

\subsection{Gene-specific technology scaling: $m_g$}

\[
m_g=\text{gene-specific scaling factor}.
\]

This is a \term{parameter}, not a hyperparameter.

Its job is to correct for this problem:

\begin{quote}
The same gene might be detected differently in the single-cell reference data and in the spatial data.
\end{quote}

For example, gene $g$ might look strong in single-cell RNA-seq but weaker in Visium spatial data. Then $m_g$ adjusts for that.

A simplified version of the prior is:
\[
m_g\sim \mathrm{Gamma}\left(\alpha_m,\frac{\alpha_m}{m_{\mathrm{mean}}}\right).
\]
Because we use the shape-rate convention,
\[
\E[m_g]=m_{\mathrm{mean}}.
\]
The model also places priors on the mean and variation-controlling variables, such as
\[
m_{\mathrm{mean}}\sim \mathrm{Gamma}(\text{hyperparameter values})
\]
and an Exponential prior on a transformed variable that controls $\alpha_m$.

Plain English:

\begin{quote}
$m_g$ is like a correction knob for each gene. The prior says the knob should usually be near a reasonable value, unless the data strongly suggest otherwise.
\end{quote}

So:
\[
m_g\text{ is a parameter.}
\]
The values controlling its Gamma prior are hyperparameters.

\subsection{Location sensitivity: $y_s$}

\[
y_s=\text{RNA detection sensitivity at location }s.
\]

This is also a \term{parameter}, not a hyperparameter.

Its job is to correct for the fact that some spatial spots capture more RNA than others. Maybe two spots contain similar cell types, but one spot has stronger sequencing signal. Then $y_s$ helps adjust for that.

The prior is
\[
y_s\sim \mathrm{Gamma}\left(\alpha^y,\frac{\alpha^y}{y_e}\right),
\]
where $y_e$ is the average detection sensitivity for experiment or batch $e$. The batch-level average also has a prior, often written as
\[
y_e\sim \mathrm{Gamma}\left(10,\frac{10}{\text{detection mean}}\right).
\]
...
\begin{quote}
Each location's detection sensitivity should usually be close to the experiment average $y_e$.
\end{quote}

The hyperparameter $\alpha^y$ controls how strict this is.

\begin{center}
\begin{tabularx}{0.95\textwidth}{>{\bfseries}p{0.28\textwidth}X}
\toprule
$\alpha^y$ value & Meaning \\
\midrule
Large, such as $200$ & Strong regularization: $y_s$ stays close to the batch average $y_e$. \\
Smaller, such as $20$ & Weaker regularization: $y_s$ can vary more across spots. \\
\bottomrule
\end{tabularx}
\end{center}
...
So:
\[
y_s\text{ is a parameter,}
\qquad
\alpha^y\text{ is a hyperparameter.}
\]

\subsection{Additive background or contamination: $s_{e,g}$}

\[
s_{e,g}=\text{extra expression of gene }g\text{ in experiment }e
\]
that cannot be explained by the cell-type mixture.

This is a \term{parameter}, not a hyperparameter.

Its job is to handle background RNA, sometimes called free-floating or contaminating RNA.

A simplified Gamma-based prior is:
\[
s_{e,g}\sim \mathrm{Gamma}\left(\alpha^s_e,\frac{\alpha^s_e}{s^{\mathrm{mean}}_e}\right).
\]
So
\[
\E[s_{e,g}]=s^{\mathrm{mean}}_e.
\]
The model also puts priors on background mean and variation-controlling variables, for example:
\[
s^{\mathrm{mean}}_e\sim \mathrm{Gamma}(a_s,b_s),
\]
with additional Gamma or Exponential hierarchy for the variation.

Plain English:

\begin{quote}
$s_{e,g}$ is the model's background-noise correction. It says: maybe some reads for gene $g$ appear even if the cell-type mixture does not fully explain them.
\end{quote}

So:
\[
s_{e,g}\text{ is a parameter.}
\]
The constants controlling its prior are hyperparameters.

\subsection{Overdispersion: $\alpha_{e,g}$}

\[
\alpha_{e,g}=\text{overdispersion parameter for gene }g\text{ in experiment }e.
\]

This is a \term{parameter}. It controls how noisy the counts are around the mean $\mu_{s,g}$.

One common way to think about the negative binomial is:
\[
\Var(d_{s,g})=\mu_{s,g}+\frac{\mu_{s,g}^2}{\alpha_{e,g}}.
\]
So smaller $\alpha_{e,g}$ means more extra noise.

The implementation uses a transformed version. Let
\[
q_{e,g}=\alpha_{e,g}^{-1/2}.
\]
Then the prior can be written as
\[
q_{e,g}\sim \mathrm{Exponential}(\phi_\alpha),
\]
with
\[
\phi_\alpha\sim \mathrm{Gamma}(9,3),
\]
and then
\[
\alpha_{e,g}=\frac{1}{q_{e,g}^2}.
\]

Plain English:

\begin{quote}
$\alpha_{e,g}$ tells the model how much random count noise to expect for each gene and experiment.
\end{quote}

So:
\[
\alpha_{e,g}\text{ is a parameter,}
\]
while
\[
\phi_\alpha\text{ and the constants in its Gamma prior are hyperparameters.}
\]

\subsection{Reference signatures: $g_{f,g}$}

\[
g_{f,g}=\text{expected expression of gene }g\text{ in cell type }f.
\]

In the page-12 spatial mapping model, $g_{f,g}$ is usually treated as \term{given input}, not as an unknown parameter being estimated in that same step.

This can feel confusing because $g_{f,g}$ was estimated earlier from single-cell or single-nucleus RNA-seq. So there are two levels of the workflow:

\begin{center}
\begin{tabularx}{0.95\textwidth}{>{\bfseries}p{0.30\textwidth}X}
\toprule
Workflow step & Role of $g_{f,g}$ \\
\midrule
Reference-building step & $g_{f,g}$ is estimated from single-cell data, usually with a count model. \\
Spatial mapping step & $g_{f,g}$ is treated as known input and used to estimate $w_{s,f}$. \\
\bottomrule
\end{tabularx}
\end{center}

So, for the page-12 spatial model:
\[
g_{f,g}\text{ is treated as known input.}
\]
But in the full workflow:
\[
g_{f,g}\text{ was estimated earlier from another model.}
\]

\subsection{Summary table: parameter or hyperparameter?}

\begin{center}
\small
\begin{tabularx}{0.98\textwidth}{>{\bfseries}p{0.13\textwidth}p{0.21\textwidth}p{0.22\textwidth}X}
\toprule
Symbol & Meaning & Role & Prior / distribution idea \\
\midrule
$d_{s,g}$ & observed gene count & data & $d_{s,g}\sim \NB(\mu_{s,g},\alpha_{e,g})$ \\
$w_{s,f}$ & cell-type abundance & parameter & hierarchical Gamma factorization prior \\
$\hat N$ & expected cells per spot & hyperparameter & controls prior for total cells and absolute abundance scale \\
$m_g$ & gene technology correction & parameter & Gamma prior, with hyperpriors on mean/variation \\
$y_s$ & spot detection sensitivity & parameter & $y_s\sim \mathrm{Gamma}(\alpha^y,\alpha^y/y_e)$ \\
$\alpha^y$ & regularization for $y_s$ & hyperparameter & controls how much $y_s$ can vary \\
$s_{e,g}$ & background RNA & parameter & Gamma prior, with hyperpriors on mean/variation \\
$\alpha_{e,g}$ & NB overdispersion & parameter & indirect Exponential/Gamma prior through $\alpha_{e,g}^{-1/2}$ \\
$g_{f,g}$ & reference signature & input here & estimated earlier, then treated as fixed input \\
\bottomrule
\end{tabularx}
\end{center}
...
The biggest idea is:
\[
\text{parameters are unknown quantities the model estimates,}
\]
while
\[
\text{hyperparameters are settings that control the priors on those unknowns.}
\]
For cell2location, the main biological parameter of interest is $w_{s,f}$. The other parameters, such as $m_g$, $y_s$, $s_{e,g}$, and $\alpha_{e,g}$, help the model avoid mistaking technical noise for real cell-type abundance.
\end{bigidea}
...
\section{How do we integrate out variables we do not care about?}

At this point, it is completely reasonable to ask:

\begin{quote}
If the full model has many unknowns, but I only care about $w_{s,f}$, what does the computer actually do with all the other unknowns?
\end{quote}

This is one of the most important ideas in Bayesian statistics. The other unknowns are often called \term{nuisance variables}. That name does \emph{not} mean they are useless. It only means:

\begin{quote}
They are not the final answer we want, but we still need to include them so the model is honest about uncertainty.
\end{quote}
...
W=\{w_{s,f}\}
\]
be the cell-type abundance map that we care about. Let
\[
\eta=\theta_{\mathrm{full}}\setminus W
\]
be all the other unknowns, such as
\[
\eta=\{n_s,b_s,z_{s,r},x_{r,f},k_r,m_g,y_s,y_e,s_{e,g},\alpha_{e,g},\ldots\}.
\]
Then
...
The Bayesian answer for only $W$ is

\begin{mathbox}
\[
p(W\mid D,H)
=
\int p(W,\eta\mid D,H)\,\dd\eta.
\]
\end{mathbox}

This operation is called \term{marginalization}. In plain English:

\begin{quote}
To answer questions about $W$, average over all believable values of the other unknowns $\eta$.
\end{quote}

\begin{warningbox}
Marginalization is \emph{not} the same as finding one best value of $\eta$ and pretending it is perfectly known. It keeps the uncertainty in $\eta$ and passes that uncertainty into the final uncertainty for $W$.
\end{warningbox}

\subsection{A tiny example where integration is just adding}

The integral looks scary because it is written for continuous variables. But the idea is simple. Imagine there is only one nuisance variable, the spot sensitivity $y_s$, and imagine it can take only two possible values. Also imagine $W$ can take only two possible values.

Suppose that, after seeing the data, the full posterior assigns the following probabilities:

\begin{center}
\begin{tabular}{ccc}
\toprule
Possible abundance $W$ & Possible sensitivity $y_s$ & Posterior probability \\
\midrule
$W=3$ cells & $y_s=0.8$ & $0.20$ \\
$W=3$ cells & $y_s=1.2$ & $0.10$ \\
$W=6$ cells & $y_s=0.8$ & $0.25$ \\
$W=6$ cells & $y_s=1.2$ & $0.45$ \\
\bottomrule
\end{tabular}
\end{center}

If we only care about $W$, we add the probabilities over the nuisance variable $y_s$:

\[
p(W=3\mid D,H)=0.20+0.10=0.30,
\]
\[
p(W=6\mid D,H)=0.25+0.45=0.70.
\]

So after averaging over $y_s$, the model says $W=6$ cells is more likely than $W=3$ cells.

\begin{bigidea}
For a small number of possibilities, marginalization means \textbf{adding over the variables you do not care about}. For continuous variables, the same idea becomes an \textbf{integral}. For huge models, the computer uses approximation methods.
\end{bigidea}

\subsection{Why not just choose the best $y_s$, $m_g$, or $n_s$?}

A tempting shortcut is:

\begin{quote}
Find the best values of all helper variables, plug them in, and then estimate $W$.
\end{quote}

That is simpler, but it can be overconfident. Here is a classroom analogy.

Suppose you want to know how many students are in a classroom, but the photo is blurry. You are also unsure about the lighting. If you choose one lighting correction and act as if it is definitely correct, your final student count may look more certain than it really is.

Bayesian inference says:

\begin{quote}
Consider many reasonable lighting corrections. Some are more believable than others. Average over them when estimating the number of students.
\end{quote}

In cell2location, this means the model should not say, ``I am 100\% sure about $y_s$, $m_g$, $n_s$, and the tissue-zone factors.'' Instead, it lets uncertainty in those quantities affect the final uncertainty in $w_{s,f}$.

\subsection{How this would work with posterior samples}

One common Bayesian strategy is \term{sampling}. The computer creates many complete ``possible worlds'' from the posterior distribution. Each world contains values for \emph{all} unknowns:

\[
(W^{(1)},\eta^{(1)}),\quad (W^{(2)},\eta^{(2)}),\quad \ldots,\quad (W^{(T)},\eta^{(T)}).
\]

A tiny fake example might look like this:

\begin{center}
\begin{tabular}{ccccc}
\toprule
Posterior world & $w_{s,f}$ & $n_s$ & $y_s$ & $m_g$ \\
\midrule
1 & 4.2 & 8.5 & 1.10 & 0.92 \\
2 & 5.1 & 9.2 & 0.97 & 1.05 \\
3 & 4.8 & 8.8 & 1.03 & 0.98 \\
4 & 5.6 & 10.0 & 0.90 & 1.08 \\
5 & 4.5 & 8.1 & 1.15 & 0.89 \\
\bottomrule
\end{tabular}
\end{center}

If we only care about $w_{s,f}$, we keep the $w_{s,f}$ column and ignore the other columns:

\[
4.2,\quad 5.1,\quad 4.8,\quad 5.6,\quad 4.5.
\]

Then the posterior mean abundance is approximated by the average:

\begin{mathbox}
\[
\widehat{\E}[w_{s,f}\mid D,H]
=
\frac{1}{T}\sum_{t=1}^{T} w_{s,f}^{(t)}.
\]
\end{mathbox}

For the tiny fake sample above,
\[
\frac{4.2+5.1+4.8+5.6+4.5}{5}=4.84.
\]

The uncertainty can also be estimated from the spread of the sampled $w_{s,f}$ values. If the samples are tightly packed, the model is confident. If they are spread out, the model is uncertain.

\begin{bigidea}
With posterior samples, ``integrating out $\eta$'' is surprisingly simple: sample all variables, then keep the $W$ values and throw away the helper-variable columns when summarizing the answer.
\end{bigidea}

\subsection{But cell2location uses variational inference}

Cell2location does not try to compute the giant integral exactly. The paper says it uses \term{approximate variational inference} to estimate the model parameters. This is important because the model has many spots, genes, cell types, and helper variables.
...
p(W,\eta\mid D,H).
\]
This distribution is too complicated to write down and use exactly. So the software chooses a simpler family of distributions, called
\[
q_\lambda(W,\eta),
\]
where $\lambda$ represents adjustable settings inside the approximation.

The goal is:

\begin{mathbox}
\[
q_\lambda(W,\eta)\approx p(W,\eta\mid D,H).
\]
\end{mathbox}
...
\begin{quote}
Find an easier distribution $q_\lambda$ that behaves as much as possible like the true posterior.
\end{quote}

The computer starts with a rough approximation and repeatedly improves it. It adjusts $\lambda$ so that the approximation gives high probability to explanations that:

\begin{itemize}
  \item predict the observed gene counts well,
  \item agree with the prior distributions reasonably well,
  \item keep track of uncertainty instead of returning only one answer.
\end{itemize}

Once the approximation is trained, the answer for the cell abundances is approximately

\begin{mathbox}
\[
p(W\mid D,H)\approx q_\lambda(W).
\]
\end{mathbox}

That means the hard integral
\[
\int p(W,\eta\mid D,H)\,\dd\eta
\]
is replaced by a practical approximation that the computer can optimize.

\subsection{A picture of what the computer is doing}

\begin{figure}[H]
\centering
\begin{tikzpicture}[node distance=0.75cm, every node/.style={font=\small,align=center}, >=Latex]
  \node[draw, rounded corners, fill=softblue, text width=3.0cm] (full) {Full posterior\\$p(W,\eta\mid D,H)$\\all unknowns together};
  \node[draw, rounded corners, fill=softgreen, right=0.8cm of full, text width=3.0cm] (approx) {Approximate it\\using samples or\\variational distribution $q$};
  \node[draw, rounded corners, fill=softorange, right=0.8cm of approx, text width=3.0cm] (drop) {Average over\\or ignore $\eta$\\when summarizing};
  \node[draw, rounded corners, fill=lightyellow, right=0.8cm of drop, text width=3.0cm] (answer) {Final answer\\posterior for\\$W=\{w_{s,f}\}$};
  \draw[->, thick] (full) -- (approx);
  \draw[->, thick] (approx) -- (drop);
  \draw[->, thick] (drop) -- (answer);
\end{tikzpicture}
\caption{Marginalization in practice. The model keeps track of many unknowns, but the final biological answer can focus on $W$ by averaging over the rest.}
\end{figure}

\subsection{What does this mean for the output you look at?}

When you look at cell2location results, you usually look at summaries of $w_{s,f}$, such as:

\begin{itemize}
  \item the posterior mean abundance of cell type $f$ at spot $s$;
  \item posterior quantiles, which describe uncertainty;
  \item maps showing where each cell type is likely to be abundant.
\end{itemize}

The helper variables still mattered. They helped the model decide which explanations were believable. But after inference, you can focus on $W$ because the uncertainty from the helper variables has already been included in the posterior calculation.

\subsection{The most important takeaway}

\begin{checkpoint}
The model does not need to give you one perfect value for every helper variable before estimating $w_{s,f}$. Instead, Bayesian inference considers many plausible values of the helper variables and averages over them. In cell2location, this averaging is done approximately using variational inference, not by hand-calculating a giant integral.
\end{checkpoint}
...
This tutorial explains the Bayesian model described in the Methods section, especially page 12, of Kleshchevnikov et al., \emph{Cell2location maps fine-grained cell types in spatial transcriptomics}, \emph{Nature Biotechnology} 40, 661--671 (2022). The tutorial is written as an educational paraphrase with simplified examples and diagrams. The new prior-distribution sections also use the public cell2location documentation and source-code math description to explain the internal Gamma and Exponential prior structure in beginner-friendly language. Version 4 adds an expanded explanation of which helper variables belong to the full parameter set and which quantities are hyperparameters or fixed model settings. Version 5 adds a high-school-level explanation of marginalization: how Bayesian inference averages over nuisance variables in practice, and how variational inference approximates that averaging for cell2location.
